...
function run(options) {
  ...
  child.on('exit', function (code, signal) {
    ...
    if (signal === 'SIGUSR2' || code === 0 || (options.restartOnCloseCode !== false && code === options.restartOnCloseCode)) {
      // this was a clean exit, so emit exit, rather than crash
      bus.emit('exit');

      // exit the monitor, but do it gracefully
      if (signal === 'SIGUSR2' || (options.restartOnCloseCode !== false && code === options.restartOnCloseCode)) {
        // restart
        restart();
